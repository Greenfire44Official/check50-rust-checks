...
def test_reject_negative():
    ...
    check50.run("./target/debug/mario").stdin("-1").reject()


@check50.check(compiles)
def test0():
    """rejects a height of 0"""
    check50.run("./target/debug/mario").stdin("0").reject()


@check50.check(compiles)
def test1():
    """handles a height of 1 correctly"""
    out = check50.run("./target/debug/mario").stdin("1").stdout()
...
def test2():
    ...
    out = check50.run("./target/debug/mario").stdin("2").stdout()
...
def test8():
    ...
    out = check50.run("./target/debug/mario").stdin("8").stdout()
...
def test9():
    ...
    out = check50.run("./target/debug/mario").stdin("-1").reject().stdin("2").stdout()
    check_pyramid(out, open("2.txt").read())
...
def test_reject_foo():
    ...
    check50.run("./target/debug/mario").stdin("foo").reject()


@check50.check(compiles)
def test_reject_empty():
    """rejects a non-numeric height of "" """
    check50.run("./target/debug/mario").stdin("").reject()
